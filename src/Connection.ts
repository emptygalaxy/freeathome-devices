...
export class Connection extends EventEmitter implements Subscriber, Logger {
  ...
  constructor(
    ...
    this.sysAccessPoint = new SystemAccessPoint(config, this, this);
  ...
  error(...messages: string[] | number[] | Record<string, any>[]): void {
    ...
    if (
      ...
      messages.length > 0 &&
      this.autoReconnect &&
      unrecoverableErrors.indexOf(messages[0].toString()) > -1
    ) {
  ...
  private breakSilence() {
    ...
    this.silenceTimeoutId = setTimeout(
      this.handleSilenceTimout.bind(this),
  ...
  private async handleSilenceTimout() {
    this.logger.log(
      "Didn't hear back for " + this.silenceTimeoutDuration + 'ms',
      'with last update on ',
      this._lastUpdate
    );
