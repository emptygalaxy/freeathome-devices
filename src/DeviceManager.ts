...
export class DeviceManager extends EventEmitter {
  ...
  private handleDevices(devices: Devices) {
    ...
    for (const deviceSerial in devices) {
      ...
      const device: Device | undefined = this.createDevice(
        info.serialNumber,
        info.deviceId
      );
      ...
      if (device !== undefined) {
        ...
        if (device instanceof HomeTouchPanel) {
          const subDevices: Device[] = (
            device as HomeTouchPanel
          ).getSubDevices();
  ...
  public getDevicesWithFunction(
    functionId: FunctionId | SupportedFunctionId
  ): Device[] {
    return this.allDevices.filter((device: Device) => {
      return (
        device instanceof SubDevice && device.getFunctionId() === functionId
      );
    });
  }
  ...
  private createDevice(
    ...
    const deviceType: DeviceType | undefined =
      DeviceManager.getDeviceType(typeId);
    switch (deviceType) {
      case DeviceType.HomeTouch:
        return new HomeTouchPanel(
          this.logger,
          this.connection,
          serialNumber,
          this.mqttClient
