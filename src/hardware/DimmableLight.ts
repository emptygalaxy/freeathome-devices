...
export class DimmableLight extends Light {
  ...
  private mininumBrightness = 1;
  private minimumBrightnessDataPoint = 'pm0001';

  private brightness = 0;
  private brightnessDataPointGet = 'odp0001';
  private brightnessDataPointSet = 'odp0001';
  ...
  public async setBrightness(brightness: number): Promise<void> {
    ...
    await this.setDatapoint(
      this.channel,
      this.brightnessDataPointSet,
      Math.round(this.brightness).toString()
    );
  }
  ...
  protected handleChannelState(datapoints: {[dp: string]: string}): void {
    ...
    if (datapoints[this.minimumBrightnessDataPoint]) {
      this.mininumBrightness = parseInt(
        datapoints[this.minimumBrightnessDataPoint]
      );
    }
  ...
  public changed(): void {
    ...
    this.logger.info(
      `[${this.getIdentifierName()}] Light brightness ` + this.brightness
    );
  }
}
