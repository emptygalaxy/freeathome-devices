...
export class Thermostat extends SubDevice {
  ...
  public async enableAutoHeaing(): Promise<void> {
    ...
    await this.setDatapoint(
      this.channel,
      this.setHeatingEnabledDataPoint,
      this.heatingEnabledValue
    );
  }
  ...
  public async setTargetTemperature(targetTemperature: number): Promise<void> {
    ...
    await this.setDatapoint(
      this.channel,
      this.setTargetTemperatureDataPoint,
  ...
  protected handleChannelState(datapoints: {[p: string]: string}) {
    ...
    this.currentHeatingEnabled =
      datapoints[this.currentHeatingEnabledDataPoint] ===
      this.heatingEnabledValue;
    this.targetHeatingEnabled =
      datapoints[this.getHeatingEnabledDataPoint] === this.heatingEnabledValue;

    this.currentTemperature = parseFloat(
      datapoints[this.currentTemperatureDataPoint]
    );
    this.targetTemperature = parseFloat(
      datapoints[this.getTargetTemperatureDataPoint]
    );
  }
  ...
  protected handleChannelUpdate(datapoints: {[p: string]: string}) {
    ...
    if (datapoints[this.currentHeatingEnabledDataPoint]) {
      const previousHeatingEnabled = this.currentHeatingEnabled;
      this.currentHeatingEnabled =
        datapoints[this.currentHeatingEnabledDataPoint] ===
        this.heatingEnabledValue;
    ...
    if (datapoints[this.getHeatingEnabledDataPoint]) {
      this.targetHeatingEnabled =
        datapoints[this.getHeatingEnabledDataPoint] ===
        this.heatingEnabledValue;
    }
