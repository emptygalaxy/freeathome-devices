...
export class AutomaticDoorOpener extends SubDevice {
  ...
  protected handleChannelState(datapoints: {[dp: string]: string}): void {
    ...
    if (datapoints[this.actuatorDatapoint] === this.activeValue) {
      ...
      this.logger.warn(
        ...
        'unknown initial actuatorDatapoint value',
        datapoints
      );
    }
  }
  ...
  protected handleChannelUpdate(datapoints: {[dp: string]: string}): void {
    ...
    if (datapoints[this.actuatorDatapoint] === this.activeValue) {
      ...
      // disabling
      this.emit(AutomaticDoorOpenerEvent.DISABLE);
    } else if (datapoints[this.sensorDatapoint] === this.activeValue) {
      // enabled confirmation
      this.enabled();
    } else if (datapoints[this.sensorDatapoint] === this.inactiveValue) {
      ...
      this.logger.warn(
        this.serialNumber,
        this.channel.toString(16),
        'unknown actuatorDatapoint value',
        datapoints
      );
    }
  }
  ...
  public changed(): void {
    ...
    this.logger.info(
      `[${this.getIdentifierName()}] Automatic door opener ` +
